# 🔧 Scripts - Initialization and Maintenance Automation

## 📋 General Overview

This directory contains **shell (bash) scripts** to automate critical initialization, configuration and maintenance tasks for the Transcendence project. The scripts manage SSL certificates, Vault initialization, ModSecurity logs, and WAF rule testing.

## 🗂️ Script Structure

```
scripts/
├── 00.gen_selfsigned_cert.sh           # Generate self-signed SSL certificates
├── 01.init_waf_and_vault.sh            # Initialize WAF and HashiCorp Vault
├── 02.read_vault_from_host.sh          # Read Vault secrets from host
├── 03.test_vault_secrets_from_container.sh  # Test Vault in container
└── 99.test_modsec_rules.sh             # ModSecurity rule testing
```

---

## 🔐 `00.gen_selfsigned_cert.sh` - SSL Certificate Generation

### Purpose

Generate **self-signed** SSL/TLS certificates for local development with HTTPS.
...
# 1. Detect certificates directory
...
EOF
  ...
  # 4. Generate self-signed certificate (valid for 365 days)
  openssl req \
    -x509 \
    -nodes \
    -days 365 \
...
make certs  # If target exists in Makefile
```
...
⚠️ Self-signed certificates will generate browser warnings:

- **Chrome/Edge**: "Your connection is not private" (NET::ERR_CERT_AUTHORITY_INVALID)
- **Firefox**: "Warning: Potential Security Risk Ahead"

**Solution**: Click "Advanced" → "Proceed to localhost (unsafe)" during development.
...
Master script that initializes **all security components**:
1. Log directories (Nginx + ModSecurity)
2. SSL certificates (calls `00.gen_selfsigned_cert.sh`)
3. HashiCorp Vault (unsealing + configuration)
4. AppRole credentials for authentication
...
SECRETS_ROOT="$BASE_DIR/secrets"  # or $SRC_DIR/secrets (legacy)
...
## 🔍 `02.read_vault_from_host.sh` - Read Secrets from Host

### Purpose

Utility script to **read Vault secrets from the host** (outside containers).
...
## 🐳 `03.test_vault_secrets_from_container.sh` - Container Test

### Purpose

Verify that the **backend can read Vault secrets using AppRole** from inside the container.
...
## 🛡️ `99.test_modsec_rules.sh` - WAF Testing

### Purpose

Run **test suites against ModSecurity** to validate rules.
...
**Location**: `/srcs/scripts/`  
**Technologies**: Bash, OpenSSL, Docker Compose, HashiCorp Vault CLI, curl  
**Related documentation**:
- `../secrets/certs/README.md` - Generated certificates
- `../secrets/vault/README.md` - Vault keys
- `../conf/modsecurity/README.md` - Tested WAF rules